...
#define GPIO_VOL_DN_KEY IMX_GPIO_NR(1, 4)
...
void lcd_enable(void)
{
	...
	*/
	...
	/* LVDS panel CABC_EN1 */
	mxc_iomux_v3_setup_pad(MX6Q_PAD_EIM_D27__GPIO_3_27);
#elif defined CONFIG_MX6DL
	/* PWM backlight */
	mxc_iomux_v3_setup_pad(MX6DL_PAD_SD1_DAT3__PWM1_PWMO);
	/* LVDS panel CABC_EN0 */
	mxc_iomux_v3_setup_pad(MX6DL_PAD_EIM_D26__GPIO_3_26);
	/* LVDS panel CABC_EN1 */
	mxc_iomux_v3_setup_pad(MX6DL_PAD_EIM_D27__GPIO_3_27);
#endif
	/*
...
*could not be power off anymore*/
	...
	/* board id for linux */
	gd->bd->bi_arch_number = MACH_TYPE_MX6Q_ZEUS;
...
int checkboard(void)
{
	printf("Board: %s-ZEUS: %s Board: 0x%x [",
	mx6_chip_name(),
	mx6_board_rev_name(),
	fsl_system_rev);
